...
# The goal for the agent is to discover wich combination of Hallucination will be better to defeat Terran
...
class SmartAgent(base_agent.BaseAgent):
    ...
    def step(self, obs):
        ...
        if not player_:
            return _NO_OP
            # player_center = numpy.mean(player_, axis=0).round()
        else:
        ...
        sentry_y, sentry_x = (unit_type == PROTOSS_SENTRY).nonzero()
        n_sentry_count = 1 if sentry_y.any() else 0

        enemy_y, enemy_x = (unit_type == TERRAN_HELLION).nonzero()
        n_enemies_count = 1 if enemy_y.any() else 0

        current_state = [
            n_sentry_count,
            hallucinations_count,  # how does this method work?
            n_enemies_count,
